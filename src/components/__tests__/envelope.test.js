...
describe(`Rendering`, ()=>{
  ...
  it(`renders JSON profile values`,()=>{
    ...
    expect(screen.getByLabelText(`volume`).value).toBe(`${profile.settings.envelope.gain}`);
  })
})
...
describe(`Interaction`,()=>{
  ...
  it(`changes attack value in profile`, ()=>{
    ...
    expect(profile.settings.envelope.a).toBe(0.0001);
    expect(slider.value).toBe(`0.0001`);
  ...
  it(`changes decay value in profile`, ()=>{
    ...
    expect(profile.settings.envelope.d).toBe(0.0001);
    expect(slider.value).toBe(`0.0001`);
  ...
  it(`changes sustain value in profile`, ()=>{
    ...
    expect(profile.settings.envelope.s).toBe(0.0001);
    expect(slider.value).toBe(`0.0001`);
  ...
  it(`changes release value in profile`, ()=>{
    ...
    expect(profile.settings.envelope.r).toBe(0.0001);
    expect(slider.value).toBe(`0.0001`);
  ...
  it(`changes gain value in profile`, ()=>{
    render(
      <Profile.Provider value={profile}>
        <Envelope/>
      </Profile.Provider>
    ...
    expect(profile.settings.envelope.gain).toBe(1);
    expect(slider.value).toBe(`1`);

    fireEvent.change(slider, {target:{value:0.5}});

    expect(profile.settings.envelope.gain).toBe(`0.5`);
    expect(slider.value).toBe(`0.5`)
    expect(Audio.setEnvelope).toBeCalled();
  })
})
